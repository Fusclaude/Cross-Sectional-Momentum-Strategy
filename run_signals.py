...
from picks_history_writer import upsert_picks
...
def main():
    ...
    # upsert, not append: re-running the workflow for a date that's already
    # in the file (manual dispatch, retry after a push conflict, etc.)
    # overwrites that date's row in place instead of adding a duplicate.
    # Makes this script idempotent, which is what removes the git-conflict
    # risk on picks_history.jsonl entirely.
    upsert_picks(history_path, snapshot)
    print(f"Upserted snapshot ({snapshot['date']}) into {history_path}")
